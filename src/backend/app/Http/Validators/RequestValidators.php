...
//            'staff_linked_id' => 'nullable|exists:users,id|unique:student_parents',
    ...
    public static function addressValidator(array $data){
        return Validator::make($data, [
            'user_id' => 'required|exists:users,id',
            'address' => 'required|string',
            'pin_code' => 'required|numeric',
            'city' => 'required|max:50|min:3',
            'district' => 'required|max:50|min:3',
            'state' => 'required|max:50|min:3',
            'country' => 'required|max:50|min:3',
            'type' => ['required', Rule::in(Constants::ADDRESS_TYPES)],
        ]);
    }

    public static function addressUpdateValidator(array $data){
        return Validator::make($data, [
            'address' => 'sometimes|string',
            'pin_code' => 'sometimes|numeric',
            'city' => 'sometimes|max:50|min:3',
            'district' => 'sometimes|max:50|min:3',
            'state' => 'sometimes|max:50|min:3',
            'country' => 'sometimes|max:50|min:3',
            'type' => ['required', Rule::in(Constants::ADDRESS_TYPES)], //updation happens based on this
        ]);
    }
}
