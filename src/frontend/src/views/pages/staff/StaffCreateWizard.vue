    ...
    <div class="mt-5">
      <form-wizard ref="fw" v-if="!creating" color="rgba(var(--vs-primary), 1)" errorColor="rgba(var(--vs-danger), 1)"
        ...
        <tab-content title="Address Details" class="mb-5" icon="feather icon-map-pin"
          ...
          <address-create-form :comm-address="commAddressData" :perm-address="permAddressData" ref="uaf"/>
        </tab-content>
  ...
  export default {
    ...
    data() {
      return {
        ...
        commAddressData: commons.getAddressModel("Communication"),
        permAddressData: commons.getAddressModel("Permanent"),
    ...
    methods: {
      ...
      createStaff() {
        ...
        let permAddressData = (this.$refs.uaf.isPermSameAsComm) ? commons.cloneObject(this.commAddressData) : this.permAddressData;
        permAddressData.type = "Permanent";

        let numOfRequests = 4 + roleData.length;
        ...
        jwt.createUser(this.staffUserModel)
          .then((userResponse) => {
            ...
            this.progressUpdate("Adding User Communication Address...", progressIncrementStep);
            jwt.createUserAddress(userResponse.data.user.id, this.commAddressData).then(() => {
            }).catch((error) => {
              this.progressError("Failed to add User Communication Address", error);
              console.log(error)
            });

            this.progressUpdate("Adding User Permanent Address...", progressIncrementStep);
            jwt.createUserAddress(userResponse.data.user.id, permAddressData).then(() => {
            }).catch((error) => {
              this.progressError("Failed to add User Permanent Address", error);
              console.log(error)
            });
            ...
            roleData.forEach(async (role) => {
              try {
                await jwt.assignRoleToUser(userResponse.data.user.id, role.id, authPassword);
